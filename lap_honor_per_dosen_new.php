...
//$tahun = $_POST["tahun"]; //$tahun = $_SESSION["tahun"];
...
if($_SESSION['hak_akses'] == 1 or $_SESSION['hak_akses'] == 3){
	...
	switch ($kalban[0]['skema']){
		...
		*/
		case "Struktural": 
			...
			$honor_Xf = $ijd[0]['imbalan_pengajaran_skema_inti_xf'];
			break;
		case "Struktural Univ": 
			$skema = "Struktural Univ";
			$tunjangan_skema = $ijd[0]['tunjangan_skema_struktural'];
			$honor_Xf = $ijd[0]['imbalan_pengajaran_skema_inti_xf'];
		case "Tugas Belajar": 
			$skema = " Tugas Belajar";
	...
	/* jika sampai dengan Januari 2018 */
	$tahun_bulan = (int) $tahun.$periode_bulan;
	if ($tahun_bulan <= 201801){
		...
		$honor_xu = $ijd[0]['imbalan_pengajaran_skema_inti_xu_beban_fak']; 
		$jumlah_ditransfer = $ijd[0]['jumlah_ditransfer_ke_dosen_beban_fak'];
	}
	...
	$pdf->Cell(3,8,':',0,0,'L');
	if ($nip == '194904081979031003') {
		$pdf->Cell(50, 8, 'Skema Inti', 0, 0, 'L');
	} else {
	...
	$pdf->Cell(3,8,':',0,0,'L');
	if ($nip == '194904081979031003') {
		$pdf->Cell(50, 8, 'Guru Besar', 0, 0, 'L');
	} else {
	...
	$pdf->Cell(3,8,':',0,0,'L');
	$dasar_kesejahteraan_setara_bhmn = (int) $ijd[0]['gaji_pokok_bhmn_beban_fak'] + (int) $ijd[0]['tambahan_dasar_kesjahteraan'];
	$pdf->Cell(20, 8, number_format($dasar_kesejahteraan_setara_bhmn), 0, 0, 'R');
	$pdf->Cell(20);
	...
	$query1 = "SELECT programstudi, program, bobotkontribusi, sksekivalen, sksdisetujui, namamatakuliah, sks, namakelas, hadiraktual, kehadiranseharusnya, honorxuskemainti, honorxfskemainti + honorxfskemalain + honorxflintasfak as honorxf, totalhonor
				FROM kalban 
				WHERE nip = '$nip' and ikuthitung = 1 and tahun = $tahun_berjalan and bulan = '$periode_bulan' and kodepdpt=0
	...
	for ($i; $i<$rows1; $i++) {	
		...
		$sksdisetujui1 = $sksdisetujui1 + $data1["$i"]["sksdisetujui"];
	}
	...
	$query2 = "SELECT programstudi, program, bobotkontribusi, sksekivalen, sksdisetujui, namamatakuliah, sks, namakelas, hadiraktual, kehadiranseharusnya, honorxuskemainti, honorxfskemainti + honorxfskemalain + honorxflintasfak as honorxf, totalhonor
				FROM kalban 
				WHERE nip = '$nip' and ikuthitung = 0 and tahun = $tahun_berjalan and bulan = '$periode_bulan' and kodepdpt=0
				ORDER BY program, programstudi, namamatakuliah, kodekelas";
	...
	for ($i; $i<$rows2; $i++) {	
		...
		$sksdisetujui2 = $sksdisetujui2 + $data2["$i"]["sksdisetujui"];
	}
	...
	$pdf->cell(15,4,number_format($sksdisetujui1,"2"),1,0,'C',true);
	$pdf->cell(120);
	...
	if ($rows2 > 0){
		...
		$pdf->cell(15,4,number_format($sksdisetujui2,"2"),1,0,'C',true);
		$pdf->cell(120);
	...
	$data = array();
	$query ='';
	if($_SESSION['hak_akses'] == 3 or $_SESSION['hak_akses'] == 1){
		
		$query = "SELECT departemen, program, jenis_bimbingan, honor
				FROM bimbingan a, organisasi b
				WHERE tahun = $tahun_berjalan and bulan = $periode_bulan and a.nip = '$nip' and kodeorganisasi = kd_organisasi"; //pake tahun berdasarkan post tapi ngaco
	} /*else {
		
		$query = "SELECT departemen, program, jenis_bimbingan, honor
				FROM bimbingan a, organisasi b
				WHERE $kodeorganisasi AND tahun = $tahun_berjalan and bulan = $periode_bulan and a.nip = '$nip' and kodeorganisasi = kd_organisasi"; //pake tahun berdasarkan post tapi ngaco 
	...
	$sql = mysql_query ($query) or die ("Something Wrong with Data Bimbingan: ".mysql_error());
	...
	//cetak honor penelitian
	include('penelitian_slip_gaji.php');
